module Pages.S.Golang.Dev exposing (Model, Msg, init, page, update, view)
...
init shared =
    let
        ...
        panel0 =
            { computeQueries = [ "repo:github\\.com/golang/go$ content:output((.|\n)* -> $author) type:commit since:\"6 months ago\" count:all" ]
            , experimentalOptions =
                Just
                    { dataPoints = Just 10
        ...
        panel1 =
            { computeQueries = [ "repo:github\\.com/golang/go$ type:commit since:\"3 months ago\" count:10000 content:output((?i)(golang|reviewed|trybot|google|trust|change|googlesource|review|result|gopher|robot|gobot|ian|lance|taylor|fixes|bryan|mills|bcmills|cherry|mui|cherry|updates|keith|randall|roland|shoemaker|cox|russ|loongson|mzh|rsc|mcintosh|damien|emmanuel|neil|odeke|robert|dneil|orijtech|github|michael|filip|gries|vals|eric|fang|alex|gmail|dempsky|austin|clement|junche|matthew|pull|pratt|tobias|knysz|mdemp|klauser|mvdan|gri)|(\\w+) -> $2) @@@ https://sourcegraph.com/search?q=context:global+repo:github%5C.com/golang/go%24+type:commit+$1&patternType=regexp" ]
            , experimentalOptions =
                Just
                    { dataPoints = Just 50
                    , sortByCount = Just True
        ...
        panel2 =
            { computeQueries = [ "since:\"2 months ago\" repo:github\\.com/golang/go$ content:output(^.*/.*\\ (.*/.*) -> $1) type:diff count:all @@@ https://sourcegraph.com/search?q=context:global+repo:github%5C.com/golang/go%24+type:diff+file:$1&patternType=literal" ]
            , experimentalOptions =
                Just
                    { dataPoints = Just 10
                    , sortByCount = Just True
...
view : Model -> View Msg
view model =
    { title = "golang/dev - codestat.dev"
    , body =
        Layout.body
            [ E.column [ E.centerX ]
                [ E.el [ Region.heading 1, Font.size 24, E.paddingEach { top = 64, right = 0, bottom = 32, left = 0 } ]
                    (E.text "Golang development stats")
                , E.el [ Region.heading 2, Font.size 20 ] (E.text "Top committers to github.com/golang/go in last 6mo")
